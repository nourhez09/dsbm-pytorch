...
def train_dsbm(dsbm_ipf, x_pairs, batch_size, inner_iters, prev_model=None, fb='', first_it=False):
    ...
    dl = iter(torch.utils.data.DataLoader(torch.utils.data.TensorDataset(*dsbm_ipf.generate_new_dataset(x_pairs, prev_model=prev_model, fb=fb, first_it=first_it)), 
...
# --------------------------
# Assume ScoreNetwork is defined in your DSBM_Gaussian file.
# For example:
# class ScoreNetwork(torch.nn.Module):
#     def __init__(self, input_dim, layer_widths, activation_fn):
#         super().__init__()
#         layers = []
#         in_dim = input_dim
...
#             in_dim = width
#         layers.append(torch.nn.Linear(in_dim, layer_widths[-1]))
#         self.net = torch.nn.Sequential(*layers)
    
#     def forward(self, x, t):
#         # Concatenate time into the input.
#         t_input = t.expand_as(x)
#         inp = torch.cat([x, t_input], dim=1)
#         return self.net(inp)

# --------------------------
# The run_experiment function (and plotting, logger, etc.) remains mostly the same,
# with modifications to compute and record the KL divergence metrics.
