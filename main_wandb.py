...
wandb.init(project="dsbm-experiment", config={
    "batch_size": 128,
    "lr": 1e-4,
    # These will be updated later per experiment run
    "num_steps": None,
    "dataset_size": None,
    "dimension": None,
    "seed": None,
...
# --------------------------
# The DSBM class with its functions remains the same.
...
# --------------------------
# The run_experiment function with modifications to compute and record the metrics.
def run_experiment(outer_iters, dataset_size_param, dimension, inner_iters=10000, seed=42, epsilon=0.01):
    ...
    # Update wandb config with experiment-specific parameters including epsilon and sigma
    wandb.config.update({
        "seed": seed,
        "dimension": dimension,
        "dataset_size": dataset_size_param,
    ...
    # We'll also store the "first" state for each process direction.
    ...
    while it <= cfg.outer_iters:
        for fb in cfg.fb_sequence:
            ...
            # Here, we compare the current process's arrival with the first marginal of the opposite process.
            if fb == 'b':
                # For the backward process, use final_state and compare with the stored first_forward_state.
                if first_forward_state is not None:
                    kl_markov = compute_gaussian_kl(first_forward_state, final_state).item()
                else:
                    kl_markov = float('nan')
                # Store the first backward state if not already stored.
                if first_backward_state is None:
                    first_backward_state = initial_state
            else:  # fb == 'f'
                # For the forward process, compare its final_state with the stored first_backward_state.
                if first_backward_state is not None:
                    kl_markov = compute_gaussian_kl(first_backward_state, final_state).item()
                else:
                    kl_markov = float('nan')
                # Store the first forward state if not already stored.
...
def plot_error_evolution(error_record, outer_iters, dataset_size_param, dimension, save_dir="plots_kl_wandb"):
    ...
    plt.title(f"Mean Error (ds: {dataset_size_param}, dim: {dimension})")
    plt.legend()
    ...
    plt.title(f"Variance Error (ds: {dataset_size_param}, dim: {dimension})")
    plt.legend()
...
def run_all_experiments(epsilon=0.01, results_filename="experiment_results_kl_wdb.json", log_filename="experiment_logs_kl.json"):
    outer_iters_list = [1000]
        ...
        for outer_iters in outer_iters_list:
            for ds in dataset_sizes:
                for dim in dimensions:
                    ...
                    # Log the experiment summary to wandb
                    wandb.log({
                        "experiment_key": key,
                        "final_mean_error": error_record["mean_error"][-1],
                        "final_var_error": error_record["var_error"][-1],
                        "epsilon": epsilon
                    })
...
#             # --- Evaluation of Reciprocal Projection ---
#             recip_z0, recip_z1 = model.generate_new_dataset(x_pairs, prev_model=prev_model, fb=fb, first_it=first_it)
#             # Choose the “reciprocal sample” based on the direction:
#             rec_sample = recip_z0 if fb == 'b' else recip_z1
...
#             # (2) KL between two consecutive Markovian projections:
#             if fb == 'b':
#                 last_backward_state = final_state
#                 if last_forward_state is not None:
#                     kl_markov = compute_gaussian_kl(last_forward_state, last_backward_state).item()
#                 else:
#                     kl_markov = float('nan')
#             else:  # fb == 'f'
#                 last_forward_state = final_state
#                 if last_backward_state is not None:
#                     kl_markov = compute_gaussian_kl(last_forward_state, last_backward_state).item()
#                 else:
#                     kl_markov = float('nan')
...
# def plot_error_evolution(error_record, outer_iters, dataset_size_param, dimension, save_dir="plots_kl"):
#     iterations = np.arange(1, len(error_record["mean_error"]) + 1)
#     plt.figure(figsize=(14, 10))
...
# def run_all_experiments(epsilon=0.01, results_filename="experiment_results_kl.json", log_filename="experiment_logs_kl.json"):
#     outer_iters_list = [1000]
#     dataset_sizes = [5000, 10000]
#     dimensions = [2, 5, 10]
